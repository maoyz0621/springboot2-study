 ...
 * Copyright 2020 Inc.
 **/
package com.myz.springboot2.mybatis.page.config.utils;
    ...
    public static void handleItem(Object item, String secretKey, boolean isEncrypted) throws IllegalAccessException {
        ...
        for (Field field : fields) {
            ...
            if (fieldType == String.class) {
                if (null != AnnotationUtils.findAnnotation(field, EncryptDecryptField.class)) {
                    // 设置private类型允许访问
                ...
                if (genericType instanceof ParameterizedType) {
                    ...
                    if (genericClazz == String.class && null != AnnotationUtils.findAnnotation(field, EncryptDecryptField.class)) {
                        field.setAccessible(Boolean.TRUE);
                        ...
                        for (Field subField : subFields) {
                            if (subField.getType() == String.class && null != AnnotationUtils.findAnnotation(subField, EncryptDecryptField.class)) {
                                field.setAccessible(Boolean.TRUE);
                                List list = (List) field.get(item);
                                if (list != null && !list.isEmpty()) {
